            ...
            <>
              {errorConnection ? <ConnectionError onRetry={getOrders} /> :
                <div className="content-wrapper mt-10 dashboard-page-theme">
                    <Header title={`Etat de Caisse (${filteredOrders.length})`} />
                          ...
                          <div className="dashboard-table-wrap" style={{ overflowY : "scroll", scrollBehavior: "inherit" }}>
                            <table id="customers" className="dashboard-orders-table">
                                        ...
                                        {Orders && Orders.map((order,index) => {                               
                                            ...
                                            // Déterminer les sources des produits de cette commande
                                            const orderSources = order.sales ? 
                                                [...new Set(order.sales.map(sale => sale.product?.product_source).filter(Boolean))] : [];
                                            
                                            // Déterminer les catégories des produits de cette commande  
                                            const orderCategoryIds = order.sales ? 
                                                [...new Set(order.sales.map(sale => sale.product?.category_id).filter(Boolean))] : [];
                                            
                                            // Récupérer les noms des catégories depuis availableCategories
                                            const orderCategories = orderCategoryIds.map(categoryId => {
                                                ...
                                                <>
                                                    ...
                                                    <td>
                                                        <div style={{ display:"flex", gap:"3px", flexWrap:"wrap", maxWidth:"150px" }}>
                                                            {orderCategories.slice(0, 2).map((category, idx) => (
                                                                <span 
                                                                    key={idx}
                                                                    style={{ 
                                                                        padding:"2px 6px", 
                                                            ...
                                                            {orderCategories.length > 2 && (
                                                                <span 
                                                                    ...
                                                                    title={`+${orderCategories.length - 2} autres catégories`}
                                                                >
                                                                    +{orderCategories.length - 2}
                                                                </span>
                                                            )}
                                                            {orderCategories.length === 0 && (
                                                                <span style={{ color:"#6c757d", fontStyle:"italic", fontSize:"12px" }}>-</span>
                                                            )}
                                                        </div>
                                                    </td>
