...
export default function AddUpdateComptable() {
  ...
  const [datacomptable,setDataComptable] = useState({});
  ...
  useEffect(() => {
    comptableId && getComptableById();
  }, [comptableId]);
  
  const getComptableById = async () => {
    setLoadingInput(true);
    axiosClient.get(`/user/${comptableId}`).then(({data}) => {
      let list = data.data;
      setDataComptable(list);
  ...
  const handleAddUpdateTackler = async(e) => {
    ...
    if (!comptableId) {
      let datauser = {lastname : inputtacklers.current[0].value.trim(), firstname : inputtacklers.current[1].value.trim(), email : inputtacklers.current[2].value.trim(), state : 'asset', password : '123456', type : 'comptable'
      ...
      await axiosClient.post('/signup',datauser).then(async ({data})  => {
        ...
        Swal.fire({position: 'Center',icon: 'success',title: 'Succès!',text: "Comptable ajoutée avec succès" ,showConfirmButton: true, confirmButtonColor: '#032546'});  
        ...
        let data = {_method : 'PUT', lastname : inputtacklers.current[0].value.trim(), firstname : inputtacklers.current[1].value.trim(), email : inputtacklers.current[2].value.trim()
            , sexe : inputtacklers.current[3].value.trim(), contact_name : inputtacklers.current[5].value.trim(), contact_cni : inputtacklers.current[6].value.trim(), contact_phone : inputtacklers.current[7].value.trim(), contact_link : inputtacklers.current[8].value.trim()};            
        await axiosClient.post(`/user/edit/${comptableId}`,data).then(({data})  => {
            Swal.fire({position: 'Center',icon: 'success',title: 'Succès!',text: "Informations de la Comptable modifiées avec succès !" ,showConfirmButton: true, confirmButtonColor: '#032546'});  
            setErrors(errors);
        }).catch(err => {
            const response = err.response;
  ...
  return (
    <AppLayout>
      ...
      <Header title={ comptableId ? "Modifier cette Comptable" : "Ajouter une Comptable"} />
        {errorConnection ?  <ConnectionError onRetry={getComptableById} /> :
        <div class="row">
          <div class="col-75">
            <form ref={formRef}>
              <div class="container-form">
                ...
                <div class="row">
                  <div class="col-75">
                    ...
                    <input type="text" disabled={loadinginput || comptableId ? 'disabled' : ''} ref={addInputsTackler} defaultValue={datacomptable ? datacomptable.lastname : "" } />
                    {errors.lastname && <span className="text-red-500">{errors.lastname}</span>}
                    <br /><br />
                  </div>
                  <div class="col-75">
                    <label for="fname">Prénom *</label>
                    <input type="text" disabled={loadinginput || comptableId ? 'disabled' : ''} ref={addInputsTackler} defaultValue={datacomptable ? datacomptable.firstname : "" } />
                    {errors.firstname && <span className="text-red-500">{errors.firstname}</span>}
                    <br /><br />
                  </div>
                  <div class="col-75">
                    <label for="fname">Adresse-Email *</label>
                    <input type="text" disabled={loadinginput || comptableId ? 'disabled' : ''} ref={addInputsTackler} defaultValue={datacomptable ? datacomptable.email : "" } />
                  ...
                  <div class="col-75">
                    ...
                    <select id="select-team-form" disabled={loadinginput ? 'disabled' : ''} ref={addInputsTackler} >
                      <option value={''}></option>
                      <option selected={datacomptable.sexe == 'masc' ? 'selected' : ''} value={'masc'}>Masculin</option>
                      <option selected={datacomptable.sexe == 'fem' ? 'selected' : ''} value={'fem'}>Feminin</option>
                    </select>
                  ...
                  <div class="col-75">
                    ...
                    <select id="select-team-form" disabled={loadinginput || comptableId ? 'disabled' : ''} ref={addInputsTackler} >
                      <option value={''}></option>
                      {shops && shops.map((shop) => (
                        <option selected={datacomptable.categoryId == shop.id ? 'selected' : ''} value={shop.id}>{shop.name}</option>
                      ))}
                    </select>
                  ...
                  <div class="col-75">
                    ...
                    <input type="text" disabled={loadinginput ? 'disabled' : ''} ref={addInputsTackler} defaultValue={datacomptable ? datacomptable.contact_name : "" } />
                    <br /><br />
                  </div>
                  <div class="col-75">
                    <label for="fname">Numero CNI  </label>
                    <input type="text" disabled={loadinginput ? 'disabled' : ''} ref={addInputsTackler} defaultValue={datacomptable ? datacomptable.contact_cni : "" } />
                    <br /><br />
                  </div>
                  <div class="col-75">
                    <label for="fname">Telephone  </label>
                    <input type="text" disabled={loadinginput ? 'disabled' : ''} ref={addInputsTackler} defaultValue={datacomptable ? datacomptable.contact_phone : "" } />
                    <br /><br />
                  </div>    
                  <div class="col-75">
                    <label for="fname">Lien   </label>
                    <input type="text" disabled={loadinginput ? 'disabled' : ''} ref={addInputsTackler} defaultValue={datacomptable ? datacomptable.contact_link : "" } />
                    <br /><br />
                  </div>                                  
